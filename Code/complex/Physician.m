...
%     end
    ...
    data=[data,[node_table{1,11},node_table{2,10},node_table{4,10},node_table{6,10},node_table{7,10},node_table{3,4}]'];

    [node_table,path_table]=heart_model(node_table,path_table);
    
 end
 end
 template=[0.5,1,0.5,0,-0.5,-1,-0.5];
 egm=zeros(5,size(data,2));
 for i=1:5
    temp=find(data(1,:)==1);
    for j=1:length(temp)
        egm(i,temp(j):temp(j)+6)=template;
    end
   
 end
 egm(2,:)=0.8.*egm(2,:)+0.5.*egm(3,:)+egm(4,:);
 egm(3:4,:)=[];
 figure
 axes('nextplot','add');
